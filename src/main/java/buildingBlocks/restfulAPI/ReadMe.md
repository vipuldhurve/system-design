...
<br><br>
## What is REST API?
<b><i>REST(Representational State Transfer)</i></b> is an architectural style and design principle that defines how APIs should look and work, while RESTful APIs are APIs that adheres to the REST principles. It allows for communication between a client and a server over HTTP protocol, using HTTPS methods(such as GET, POST, PUT, DELETE) to perform CRUD operations on a resource.
